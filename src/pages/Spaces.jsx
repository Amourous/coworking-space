  ...
  return (
    <div className="section-container pt-8 md:pt-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
        <div>
          <h1 className="text-4xl font-bold mb-4">{t('spaces.title')}</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-textMuted w-4 h-4 rtl:left-auto rtl:right-3" />
            <input 
              type="text" 
              placeholder={t('spaces.search')}
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="input-field pl-9 pr-3 rtl:pl-3 rtl:pr-9 w-full sm:w-64 text-sm py-2"
            />
          </div>
          <div className="relative min-w-[140px]">
            <Filter className="absolute left-3 top-1/2 -translate-y-1/2 text-textMuted w-4 h-4 rtl:left-auto rtl:right-3" />
            <select 
              value={filterType}
              onChange={(e) => setFilterType(e.target.value)}
              className="input-field pl-9 rtl:pl-3 rtl:pr-9 w-full appearance-none text-sm py-2"
            >
              <option value="all">{t('spaces.allTypes')}</option>
